...
async function main() {
  ...
  if (!auth.access_token) { console.log('Sign-in failed:', signIn.data.substring(0, 300)); return; }
  console.log('   OK! User:', auth.user.email);
  const token = auth.access_token;

  // Test auth.profile
  console.log('2. GET auth.profile...');
  const prof = await get('wassel-alpha.vercel.app', '/api/trpc/auth.profile', { 'Authorization': 'Bearer ' + token });
  console.log('   Status:', prof.status, '| Body:', prof.data.substring(0, 400));

  // Test campaign.previewMessages
  ...
  console.log('   Status:', camp.status, '| Body:', camp.data.substring(0, 500));
}
