...
echo === Building with minimal API === > deploy-log4.txt
call npm run build >> deploy-log4.txt 2>&1
echo BUILD=%errorlevel% >> deploy-log4.txt
"C:\Program Files\Git\cmd\git.exe" add -A >> deploy-log4.txt 2>&1
"C:\Program Files\Git\cmd\git.exe" commit -m "test: minimal serverless handler to debug FUNCTION_INVOCATION_FAILED" >> deploy-log4.txt 2>&1
echo === Deploying === >> deploy-log4.txt
