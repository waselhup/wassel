...
var import_express17 = require("@trpc/server/adapters/express");
...
var supabase = (0, import_supabase_js2.createClient)(supabaseUrl, supabaseServiceKey, {
  auth: {
...
router2.get("/start", async (req, res) => {
  try {
    ...
    const state = (0, import_crypto2.randomBytes)(32).toString("hex");
...
function getSupabase2() {
  ...
  return (0, import_supabase_js3.createClient)(url, key, { auth: { autoRefreshToken: false, persistSession: false } });
}
function getConfig2() {
  return {
...
router3.get("/connect", (req, res) => {
  ...
  const state = import_crypto3.default.randomUUID();
...
router4.post("/send", async (req, res) => {
  try {
    ...
    const inviteToken = (0, import_crypto4.randomBytes)(32).toString("hex");
    ...
    const errorId = (0, import_crypto4.randomBytes)(4).toString("hex");
...
function supabase2() {
  return (0, import_supabase_js4.createClient)(
    process.env.SUPABASE_URL || "",
    process.env.SUPABASE_SERVICE_ROLE_KEY || "",
    { auth: { autoRefreshToken: false, persistSession: false } }
  );
...
router9.get("/debug", async (req, res) => {
  try {
    ...
    const { data, error, count } = await supabase.from("activity_logs").select("*", { count: "exact" }).eq("team_id", teamId || "").order("created_at", { ascending: false }).limit(1);
...
router11.post("/import", async (req, res) => {
  try {
    ...
    for (let i = 0; i < leads.length; i += BATCH) {
      ...
      const { error } = await supabase.from("leads").upsert(batch, { onConflict: "team_id,linkedin_url", ignoreDuplicates: true });
...
var authRouter = router15({
   ...
   */
  onboard: protectedProcedure.input(import_zod2.z.object({
    ...
    try {
      ...
      const teamId = (0, import_crypto5.randomUUID)();
...
var demoRouter = router15({
  ...
  createDemoCampaign: protectedProcedure.mutation(async ({ ctx }) => {
    ...
    try {
      ...
      const campaignId = (0, import_crypto6.randomUUID)();
  ...
  createDemoLeads: protectedProcedure.mutation(async ({ ctx }) => {
    ...
    try {
      ...
      const campaignId = campaigns?.[0]?.id || (0, import_crypto6.randomUUID)();
      const demoLeads = [
        {
...
var campaignsRouter = router15({
   ...
   */
  list: protectedProcedure.input(import_zod3.z.object({ clientId: import_zod3.z.string().optional() }).optional()).query(async ({ ctx, input }) => {
    if (!ctx.user?.id) {
      return [];
    }
    try {
  ...
  create: protectedProcedure.input(
    import_zod3.z.object({
      name: import_zod3.z.string().min(1, "\u0627\u0633\u0645 \u0627\u0644\u062D\u0645\u0644\u0629 \u0645\u0637\u0644\u0648\u0628").max(255),
      description: import_zod3.z.string().optional(),
      type: import_zod3.z.string().default("combined")
    })
  ).mutation(async ({ ctx, input }) => {
    if (!ctx.user?.id) {
   ...
   */
  get: protectedProcedure.input(import_zod3.z.object({ id: import_zod3.z.string() })).query(async ({ ctx, input }) => {
    if (!ctx.user?.id) {
      return null;
    }
    try {
  ...
  updateStatus: protectedProcedure.input(
    import_zod3.z.object({
      ...
      status: import_zod3.z.enum(["draft", "active", "paused", "completed", "archived"])
    })
  ).mutation(async ({ ctx, input }) => {
    if (!ctx.user?.id) {
      throw new Error("\u0627\u0644\u0645\u0633\u062A\u062E\u062F\u0645 \u063A\u064A\u0631 \u0645\u0635\u0631\u062D");
   ...
   * Delete a campaign
   */
  delete: protectedProcedure.input(import_zod3.z.object({ id: import_zod3.z.string() })).mutation(async ({ ctx, input }) => {
    if (!ctx.user?.id) {
      throw new Error("\u0627\u0644\u0645\u0633\u062A\u062E\u062F\u0645 \u063A\u064A\u0631 \u0645\u0635\u0631\u062D");
    }
    try {
...
var leadsRouter = router15({
  list: protectedProcedure.input(import_zod4.z.object({ campaignId: import_zod4.z.string().optional() }).optional()).query(async ({ ctx, input }) => {
    if (!ctx.user?.id) return [];
    const teamId = await getUserTeamId(ctx.user.id);
    if (!teamId) return [];
  ...
  importLeads: protectedProcedure.input(
    import_zod4.z.object({
      campaignId: import_zod4.z.string(),
      leads: import_zod4.z.array(
        import_zod4.z.object({
          linkedin_url: import_zod4.z.string().url(),
          first_name: import_zod4.z.string().optional(),
          last_name: import_zod4.z.string().optional(),
          company: import_zod4.z.string().optional(),
          headline: import_zod4.z.string().optional(),
  ...
  updateStatus: protectedProcedure.input(import_zod4.z.object({ leadId: import_zod4.z.string(), status: import_zod4.z.string() })).mutation(async ({ ctx, input }) => {
    if (!ctx.user?.id) return null;
    const teamId = await getUserTeamId(ctx.user.id);
  ...
  delete: protectedProcedure.input(import_zod4.z.object({ leadId: import_zod4.z.string() })).mutation(async ({ ctx, input }) => {
    if (!ctx.user?.id) return null;
    const teamId = await getUserTeamId(ctx.user.id);
    if (!teamId) return null;
...
var templatesRouter = router15({
  ...
  create: protectedProcedure.input(
    import_zod5.z.object({
      name: import_zod5.z.string().min(1, "\u0627\u0633\u0645 \u0627\u0644\u0642\u0627\u0644\u0628 \u0645\u0637\u0644\u0648\u0628"),
      category: import_zod5.z.string().optional(),
      subject: import_zod5.z.string().optional(),
      content: import_zod5.z.string().min(1, "\u0645\u062D\u062A\u0648\u0649 \u0627\u0644\u0642\u0627\u0644\u0628 \u0645\u0637\u0644\u0648\u0628"),
      variables: import_zod5.z.array(import_zod5.z.string()).optional()
    })
  ).mutation(async ({ ctx, input }) => {
    if (!ctx.user?.id) {
      throw new Error("\u063A\u064A\u0631 \u0645\u0635\u0631\u062D");
  ...
  update: protectedProcedure.input(
    import_zod5.z.object({
      templateId: import_zod5.z.string(),
  ...
  delete: protectedProcedure.input(import_zod5.z.object({ templateId: import_zod5.z.string() })).mutation(async ({ ctx, input }) => {
    if (!ctx.user?.id) {
      throw new Error("\u063A\u064A\u0631 \u0645\u0635\u0631\u062D");
    }
    const teamId = await getUserTeamId(ctx.user.id);
  ...
  preview: protectedProcedure.input(
    import_zod5.z.object({
      ...
      variables: import_zod5.z.record(import_zod5.z.string(), import_zod5.z.any()).optional()
    })
  ).query(async ({ input }) => {
  ...
  linkToCampaign: protectedProcedure.input(
    import_zod5.z.object({
      templateId: import_zod5.z.string(),
      campaignId: import_zod5.z.string()
    })
  ).mutation(async ({ ctx, input }) => {
    if (!ctx.user?.id) {
      throw new Error("\u063A\u064A\u0631 \u0645\u0635\u0631\u062D");
...
var extensionRouter = router15({
  ...
  campaignCreate: protectedProcedure.input(
    import_zod6.z.object({
      name: import_zod6.z.string().min(1, "\u0627\u0633\u0645 \u0627\u0644\u062D\u0645\u0644\u0629 \u0645\u0637\u0644\u0648\u0628"),
      type: import_zod6.z.enum(["invitation", "message", "sequence"]),
      configuration: import_zod6.z.record(import_zod6.z.string(), import_zod6.z.unknown()).optional()
    })
  ).mutation(async ({ ctx, input }) => {
    if (!ctx.user?.id) {
      throw new Error("\u063A\u064A\u0631 \u0645\u0635\u0631\u062D");
  ...
  campaignGet: protectedProcedure.input(import_zod6.z.object({ campaignId: import_zod6.z.string() })).query(async ({ ctx, input }) => {
    if (!ctx.user?.id) {
      throw new Error("\u063A\u064A\u0631 \u0645\u0635\u0631\u062D");
    }
    const teamId = await getUserTeamId(ctx.user.id);
  ...
  leadsList: protectedProcedure.input(import_zod6.z.object({ campaignId: import_zod6.z.string() })).query(async ({ ctx, input }) => {
    if (!ctx.user?.id) {
      throw new Error("\u063A\u064A\u0631 \u0645\u0635\u0631\u062D");
    }
    const teamId = await getUserTeamId(ctx.user.id);
  ...
  queueList: protectedProcedure.input(import_zod6.z.object({ status: import_zod6.z.enum(["pending", "approved", "rejected"]).optional() })).query(async ({ ctx, input }) => {
    if (!ctx.user?.id) {
      throw new Error("\u063A\u064A\u0631 \u0645\u0635\u0631\u062D");
    }
    const teamId = await getUserTeamId(ctx.user.id);
  ...
  // Queue: approve item
  queueApprove: protectedProcedure.input(import_zod6.z.object({ itemId: import_zod6.z.string() })).mutation(async ({ ctx, input }) => {
    if (!ctx.user?.id) {
      throw new Error("\u063A\u064A\u0631 \u0645\u0635\u0631\u062D");
    }
    const teamId = await getUserTeamId(ctx.user.id);
  ...
  // Queue: reject item
  queueReject: protectedProcedure.input(import_zod6.z.object({ itemId: import_zod6.z.string() })).mutation(async ({ ctx, input }) => {
    if (!ctx.user?.id) {
      throw new Error("\u063A\u064A\u0631 \u0645\u0635\u0631\u062D");
    }
    const teamId = await getUserTeamId(ctx.user.id);
  ...
  templatesGet: protectedProcedure.input(import_zod6.z.object({ templateId: import_zod6.z.string() })).query(async ({ ctx, input }) => {
    if (!ctx.user?.id) {
      throw new Error("\u063A\u064A\u0631 \u0645\u0635\u0631\u062D");
    }
    const teamId = await getUserTeamId(ctx.user.id);
...
var billingRouter = router15({
   ...
   */
  requestUpgrade: protectedProcedure.input(import_zod7.z.object({ targetPlan: import_zod7.z.enum(["pro", "agency"]) })).mutation(async ({ ctx, input }) => {
    if (!ctx.user?.id) {
      throw new Error("\u0627\u0644\u0645\u0633\u062A\u062E\u062F\u0645 \u063A\u064A\u0631 \u0645\u0635\u0631\u062D");
    }
    try {
...
var appRouter = router15({
  ...
  queue: router15({
    list: protectedProcedure.input(import_zod8.z.object({ status: import_zod8.z.enum(["all", "new", "approved"]).optional() }).optional()).query(async ({ ctx, input }) => {
      if (!ctx.user?.id) return [];
      const teamId = await getUserTeamId(ctx.user.id);
      if (!teamId) return [];
    ...
    approve: protectedProcedure.input(import_zod8.z.object({ itemId: import_zod8.z.string() })).mutation(async ({ ctx, input }) => {
      if (!ctx.user?.id) return null;
      const teamId = await getUserTeamId(ctx.user.id);
      if (!teamId) return null;
    ...
    reject: protectedProcedure.input(import_zod8.z.object({ itemId: import_zod8.z.string() })).mutation(async ({ ctx, input }) => {
      if (!ctx.user?.id) return null;
      const teamId = await getUserTeamId(ctx.user.id);
      if (!teamId) return null;
      const { data } = await supabase.from("action_queue").update({
    ...
    // Bulk operations
    bulkApprove: protectedProcedure.input(import_zod8.z.object({ itemIds: import_zod8.z.array(import_zod8.z.string()) })).mutation(async ({ ctx, input }) => {
    ...
    bulkReject: protectedProcedure.input(import_zod8.z.object({ itemIds: import_zod8.z.array(import_zod8.z.string()) })).mutation(async ({ ctx, input }) => {
      if (!ctx.user?.id || input.itemIds.length === 0) return null;
      const teamId = await getUserTeamId(ctx.user.id);
      if (!teamId) return null;
      const { data } = await supabase.from("action_queue").update({
...
function getServiceSupabase() {
  ...
  return (0, import_supabase_js5.createClient)(supabaseUrl2, supabaseServiceKey2, {
    auth: { autoRefreshToken: false, persistSession: false }
  });
}
...
function verifyExtensionToken(token) {
  try {
    ...
    const secret = process.env.SUPABASE_SERVICE_ROLE_KEY || "wassel-ext-secret";
    const expectedSig = crypto2.createHmac("sha256", secret).update(payloadBase64).digest("base64url");
...
function getSupabase3() {
  return (0, import_supabase_js6.createClient)(
...
app.use(
  "/api/trpc",
  (0, import_express17.createExpressMiddleware)({
    router: appRouter,
    createContext
  })
);
