...
// Sign in to hiqotmimlgsrsnovtopd, then call live API
...
async function main() {
  // Step 1: Sign in
  console.log('1. Signing in to hiqotmimlgsrsnovtopd...');
  ...
  if (!auth.access_token) {
    console.log('Sign-in failed:', signIn.data.substring(0, 200));
    return;
  }
  console.log('   Got token:', auth.access_token.substring(0, 30) + '...');

  // Step 2: Call live API auth.profile
  console.log('2. Calling live API auth.profile...');
  const profile = await get('wassel-alpha.vercel.app', '/api/trpc/auth.profile', {
    'Authorization': 'Bearer ' + auth.access_token
  });
  console.log('   Status:', profile.status);
  console.log('   Response:', profile.data.substring(0, 300));

  // Step 3: Test campaign.previewMessages
  console.log('3. Testing campaign.previewMessages...');
  ...
  console.log('   Response:', campaign.data.substring(0, 500));
}
