...
fs.writeFileSync('client/src/components/landing/VideoDemo.tsx', component, 'utf8');
console.log('VideoDemo.tsx rewritten');

// Verify LandingPage has VideoDemo
let lp = fs.readFileSync('client/src/pages/LandingPage.tsx', 'utf8');
if (lp.charCodeAt(0) === 0xFEFF) lp = lp.slice(1);
console.log('LandingPage has VideoDemo import: ' + lp.includes('VideoDemo'));
console.log('LandingPage has <VideoDemo: ' + lp.includes('<VideoDemo'));

// Check the import path matches
if (lp.includes('VideoDemo')) {
  const importLine = lp.split('\n').find(l => l.includes('VideoDemo'));
  console.log('Import line: ' + (importLine ? importLine.trim() : 'none'));
}